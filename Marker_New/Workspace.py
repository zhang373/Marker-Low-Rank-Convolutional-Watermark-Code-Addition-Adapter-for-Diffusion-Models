...
class CodeEmb_Net(nn.Module):
    def __init__(self, Code, batchsize=1, Adj_Code_hid_dim_1=30, Adj_Code_hid_dim_2=300):
        super().__init__()
        self.Code = [[]]
    ...
    def __init__(self, use_conv, Cr_channels_in, Adj_Cr_channels_Low, Code_dim=900, Adj_Cr_n_heads=8,
        ...
        if use_conv:
            self.op_down = conv_nd(
                self.dims, self.Cr_channels_in, self.Adj_Cr_channels_Low, 3, stride=self.stride, padding=padding
            )
            ...
            self.op_mid = conv_nd(
                self.dims, self.Adj_Cr_channels_Low, self.Adj_Cr_channels_Low, 3, stride=self.stride, padding=padding
            )
            self.op_up = conv_nd(
                self.dims, self.Adj_Cr_channels_Low, self.Cr_channels_in, 3, stride=self.stride, padding=padding
            )
        else:
...
class Marker_pretrain_Net_8(nn.Module):
    def __init__(self, Channel_in, Channel_mid,  Adj_Cr_channels_Low, layernum=8):
        super(Marker_pretrain_Net_8, self).__init__()
        #self.Code = Code
        self.Adj_Cr_channels_Low = Adj_Cr_channels_Low
        ...
        self.Down_Net_4 = CrossLowR_Net(True, self.Channel_mid * self.ChannelScaleFactor[1],
                                        self.Adj_Cr_channels_Low)
        self.DownSample_Net_3 = Downsample(self.Channel_mid * self.ChannelScaleFactor[1], True,
        ...
        # For Mid block
        self.Mid_Net_1 = CrossLowR_Net(True, self.Channel_mid *
                                       self.ChannelScaleFactor[2], self.Adj_Cr_channels_Low)
        self.Mid_Net_2 = CrossLowR_Net(True, self.Channel_mid *
                                       self.ChannelScaleFactor[2], self.Adj_Cr_channels_Low)
    ...
    def CodeAcc_feature_loss(self, x_str, code, Decoder_Net_list):
        ...
        for i in range(8):
            if i<4:
                C_decoded = Decoder_Net_list[i].forward(x_str[i+1])
                loss+= BCE_loss_fn(code,C_decoded)
            else:
                C_decoded = Decoder_Net_list[7-i].forward(x_str[i+1])
                loss += BCE_loss_fn(code, C_decoded)
        return loss
...
class Marker_pretrain_Net_3(nn.Module):
    def __init__(self, Channel_in, Channel_mid,  Adj_Cr_channels_Low, layernum=3):
        super(Marker_pretrain_Net_3, self).__init__()
        #self.Code = Code
        self.Adj_Cr_channels_Low = Adj_Cr_channels_Low
        self.Channel_in = Channel_in
        self.Channel_mid = Channel_mid
        ...
        self.DownSample_Net_3 = Downsample(self.Channel_mid * self.ChannelScaleFactor[1], True,
                                           dims=2, out_channels=self.Channel_mid * self.ChannelScaleFactor[2])
        self.input_blocks = nn.ModuleList([self.Down_Net_1,self.DownSample_Net_1,self.DownSample_Net_2,
                                           self.Down_Net_4,self.DownSample_Net_3])

        # For Mid block
        self.Mid_Net_1 = CrossLowR_Net(True, self.Channel_mid *
                                       self.ChannelScaleFactor[2], self.Adj_Cr_channels_Low)
        #self.Mid_Net_2 = CrossLowR_Net(True, self.Channel_mid * self.ChannelScaleFactor[2], self.Adj_Cr_channels_Low)
        self.mid_blocks = nn.ModuleList([self.Mid_Net_1])
    ...
    def CodeAcc_feature_loss(self, x_str, code, Decoder_Net_list):
        ...
        for i in range(3):
            if i <= 1:
                ...
                C_decoded = Decoder_Net_list[2-i].forward(x_str[i+1])
                loss += BCE_loss_fn(code, C_decoded)
        return loss
...
class Marker_pretrain_Net_5(nn.Module):
    def __init__(self, Channel_in, Channel_mid,  Adj_Cr_channels_Low, layernum=5):
        super(Marker_pretrain_Net_5, self).__init__()
        #self.Code = Code
        self.Adj_Cr_channels_Low = Adj_Cr_channels_Low
        self.Channel_in = Channel_in
        self.Channel_mid = Channel_mid
        ...
        self.DownSample_Net_3 = Downsample(self.Channel_mid * self.ChannelScaleFactor[1], True,
                                           dims=2, out_channels=self.Channel_mid * self.ChannelScaleFactor[2])
        self.input_blocks = nn.ModuleList([self.Down_Net_1,self.DownSample_Net_1,
                                           self.Down_Net_3,self.DownSample_Net_2,
                                           self.Down_Net_4,self.DownSample_Net_3])
    ...
    def CodeAcc_feature_loss(self, x_str, code, Decoder_Net_list):
        ...
        for i in range(5):
            if i<3:
                ...
                C_decoded = Decoder_Net_list[4-i].forward(x_str[i+1])
                loss += BCE_loss_fn(code, C_decoded)
        return loss
